...
While it was primarily created for AtlasXCORD based on Vencord and its respective packages, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @AtlasXCORD/discord-types
yarn add -D @AtlasXCORD/discord-types
pnpm add -D @AtlasXCORD/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@AtlasXCORD/discord-types";
...
```ts
import { ApplicationCommandType } from "@AtlasXCORD/discord-types/enums";
