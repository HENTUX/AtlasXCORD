...
export const AtlasXCORDDevs = {} as Record<string, Dev>;
...
export function parseAtlasXCORDDevs() {
    const file = createSourceFile("constants.ts", readFileSync("src/utils/constants.ts", "utf8"), ScriptTarget.Latest);

    for (const child of file.getChildAt(0).getChildren()) {
        if (!isVariableStatement(child)) continue;

        const devsDeclaration = child.declarationList.declarations.find(d => hasName(d, "AtlasXCORDDevs"));
        if (!devsDeclaration?.initializer || !isCallExpression(devsDeclaration.initializer)) continue;

        const value = devsDeclaration.initializer.arguments[0];

        if (!isSatisfiesExpression(value) || !isObjectLiteralExpression(value.expression)) throw new Error("Failed to parse AtlasXCORDDevs: not an object literal");

        for (const prop of value.expression.properties) {
            const name = (prop.name as Identifier).text;
            const value = isPropertyAssignment(prop) ? prop.initializer : prop;

            if (!isObjectLiteralExpression(value)) throw new Error(`Failed to parse AtlasXCORDDevs: ${name} is not an object literal`);
    ...
    throw new Error("Could not find AtlasXCORDDevs constant");
}
            ...
            switch (key) {
                ...
                case "authors":
                    ...
                    data.authors = value.elements.map(e => {
                        ...
                        const d = devs[getName(e)!] || AtlasXCORDDevs[getName(e)!];
