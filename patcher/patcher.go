...
var AtlasC2Directory string

var ErrAlreadyReported = errors.New("already reported")

func init() {
	if dir := os.Getenv("ATLAS2C_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using ATLAS2C_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../AtlasC2Data")
		BaseDir = path.Join(dir, "..", "AtlasC2Data")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("AtlasC2").UserConfig()
	}
	dir := os.Getenv("ATLAS2C_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using ATLAS2C_DIRECTORY")
		AtlasC2Directory = dir
	} else {
		AtlasC2Directory = path.Join(BaseDir, "atlas2c.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := os.Rename(appAsar, _appAsar); err != nil {
		// Try copying instead of renaming if file is locked
		Log.Warn("Rename failed, trying copy approach:", err)
		if copyErr := CopyFile(appAsar, _appAsar); copyErr != nil {
	...
	if err := WriteAppAsar(appAsar, AtlasC2Directory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", AtlasC2Directory+"...")
		...
		args = append(args, "override", name, "--filesystem="+AtlasC2Directory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + AtlasC2Directory + ": " + err.Error())
		}
	}
	return nil
}
...
func isAtlas2cLoaderAppAsar(appAsar string) (bool, error) {
...
func cleanupDesyncedPatchedInstall(dir string, isSystemElectron bool) (bool, error) {
	...
	isLoader, err := isAtlas2cLoaderAppAsar(appAsar)
	if err != nil {
		return false, err
	}
	if isLoader {
	...
	Log.Warn("Detected a patched install with a non-AtlasC2 app.asar. Discord was likely updated while patched; removing stale _app.asar")
