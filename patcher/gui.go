...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest AtlasC2 builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				DiscordYellow,
				"**GitHub** is the only official place to get AtlasC2. Any other site claiming to be us is malicious.\n"+
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/4, 50),
						Tooltip("Reinstall & Update AtlasC2"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify AtlasC2 installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise AtlasC2 will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"AtlasC2 is in no way affiliated with OpenAsar.\n"+
...
func loop() {
	...
	g.SingleWindow().
		...
		Layout(
			...
			g.Style().SetFontSize(20).To(
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "AtlasC2 will be downloaded to: ")+AtlasC2Directory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(AtlasC2Directory))
							}),
						),
				),
				&CondWidget{!IsDevInstall, func() g.Widget {
					return g.Label("To customise this location, set the environment variable 'ATLAS2C_USER_DATA_DIR' and restart me").Wrapped(true)
				}, nil},
				g.Dummy(0, 10),
				g.Label("AtlasC2 Version: "+buildinfo.InstallerTag+" ("+buildinfo.InstallerGitHash+")"+Ternary(IsSelfOutdated, " - OUTDATED", "")),
				g.Label("Local AtlasC2 Version: "+InstalledHash),
				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating AtlasC2 due to being in DevMode")
						}
						return g.Label("Latest AtlasC2 Version: " + LatestHash)
					}, func() g.Widget {
