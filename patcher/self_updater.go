...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/HENTUX/userplugins/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "AtlasC2Cli.exe", "AtlasC2.exe")
		return BaseUrl + filename
	case "darwin":
		switch runtime.GOARCH {
		case "amd64":
			return BaseUrl + "AtlasC2-darwin-x64.zip"
		case "arm64":
			return BaseUrl + "AtlasC2-darwin-arm64.zip"
		default:
			return ""
		}
	case "linux":
		return BaseUrl + "AtlasC2Cli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "AtlasC2Update")
	if err != nil {
