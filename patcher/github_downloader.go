...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("ATLAS2C_DEV_INSTALL") == "1"
	...
	AtlasC2File := AtlasC2Directory

	stat, err := os.Stat(AtlasC2File)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		AtlasC2File = path.Join(AtlasC2File, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(AtlasC2File)
	if err != nil {
		return
	}

	Log.Debug("Found existing AtlasC2 Install. Checking for hash...")

	re := regexp.MustCompile(`// AtlasC2 (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(AtlasC2Directory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", AtlasC2Directory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", AtlasC2Directory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(AtlasC2Directory)
