...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install AtlasC2")
	var updateFlag = flag.Bool("repair", false, "Repair AtlasC2")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall AtlasC2")
	...
	if *versionFlag {
		fmt.Println("AtlasC2 CLI", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 HENTUX and AtlasC2 contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update AtlasC2' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest AtlasC2 files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, AtlasC2 will likely not work.")
}
