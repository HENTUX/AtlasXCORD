...
import { platformName } from "@equicordplugins/AtlasXCORDHelper/utils";
import customIdle from "@plugins/customIdle";
import { gitHash, gitHashShort } from "@shared/vencordUserAgent";
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, AtlasXCORD_TEAM, GUILD_ID, SUPPORT_CHANNEL_ID, SUPPORT_CHANNEL_IDS, VC_CONTRIB_ROLE_ID, VC_DONOR_ROLE_ID, VC_GUILD_ID, VC_REGULAR_ROLE_ID, VENCORD_CONTRIB_ROLE_ID } from "@utils/constants";
import { sendMessage } from "@utils/discord";
import { Logger } from "@utils/Logger";
import { Margins } from "@utils/margins";
import { isAnyPluginDev, isAtlasXCORDGuild, isAtlasXCORDSupport, isKnownIssuesCategory, isSupportChannel, tryOrElse } from "@utils/misc";
...
const TrustedRolesIds = [
    ...
    AtlasXCORD_TEAM, // AtlasXCORD Team
    DONOR_ROLE_ID, // AtlasXCORD Donor
    CONTRIB_ROLE_ID, // AtlasXCORD Contributor
...
async function generateDebugInfoMessage() {
    ...
    clientString += `${clientInfo.shortHash ? ` â€¢ [${clientInfo.shortHash}](<https://github.com/HENTUX/Equibop/commit/${clientInfo.hash}>)` : ""}`;
    ...
    const info = {
        AtlasXCORD:
            `v${VERSION} â€¢ [${gitHashShort}](<https://github.com/HENTUX/AtlasXCORD/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "AtlasXCORD DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Paragraph>You are using a custom build of AtlasXCORD, which we do not provide support for!</Paragraph>

                <Paragraph className={Margins.top8}>
                    We only provide support for <Link href="https://AtlasXCORD.org/download">official builds</Link>.
                    Either <Link href="https://AtlasXCORD.org/download">switch to an official build</Link> or figure your issue out yourself.
                </Paragraph>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "AtlasXCORD-debug",
            description: "Send AtlasXCORD debug info",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isAtlasXCORDGuild(ctx?.guild?.id, true),
            execute: async () => ({ content: await generateDebugInfoMessage() })
        },
        {
            name: "AtlasXCORD-plugins",
            description: "Send AtlasXCORD plugin list",
            // @ts-ignore
            predicate: ctx => isAnyPluginDev(UserStore.getCurrentUser()?.id) || isAtlasXCORDGuild(ctx?.guild?.id, true),
            execute: async () => {
        ...
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Paragraph>You are using an outdated version of AtlasXCORD! Chances are, your issue is already fixed.</Paragraph>
                                <Paragraph className={Margins.top8}>
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Paragraph>You are using an externally updated AtlasXCORD version, which we do not provide support for!</Paragraph>
                            <Paragraph className={Margins.top8}>
                                Please either switch to an <Link href="https://AtlasXCORD.org/download">officially supported version of AtlasXCORD</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        const AtlasXCORDSupport = isAtlasXCORDSupport(props.message.author.id);

        const shouldAddUpdateButton =
            !IS_UPDATER_DISABLED
            && ((isSupportChannel(props.channel.id) && AtlasXCORDSupport))
        ...
        if (AtlasXCORDSupport && isSupportChannel(props.channel.id) && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/AtlasXCORD-debug") || props.message.content.includes("/AtlasXCORD-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
                        variant="secondary"
                    ...
                    >
                        Run /AtlasXCORD-plugins
                    </Button>
                );
            }
        }

        if (AtlasXCORDSupport || (isSupportChannel(props.channel.id) || isKnownIssuesCategory(props.channel.parent_id))) {
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message AtlasXCORD & Vencord plugin developers for support!
                <br />
