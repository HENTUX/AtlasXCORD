...
async function fetchNowPlaying(signal?: AbortSignal): Promise<NdTrack | null> {
    ...
    try {
        ...
        const queryParams = `u=${encodeURIComponent(nd_username)}&t=${token}&s=${salt}&v=1.12.0&c=AtlasXCORD-rpc&f=json`;
...
async function getActivity(signal?: AbortSignal): Promise<Activity | null> {
    ...
    if (albumArtMode === "instance" && track.coverArt && externalBaseUrl) {
        ...
        const salt = Math.random().toString(36).substring(2, 8);
        const token = md5Hex((nd_password ?? "") + salt);
        resolvedCoverArtUrl = `${externalBaseUrl}/rest/getCoverArt?id=${encodeURIComponent(track.coverArt)}&u=${encodeURIComponent(nd_username ?? "")}&t=${token}&s=${salt}&v=1.12.0&c=AtlasXCORD-rpc`;
