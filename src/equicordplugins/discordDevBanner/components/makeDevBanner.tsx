...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, AtlasXCORDIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {AtlasXCORDIcon} {AtlasXCORDName} {AtlasXCORDVersion} ({AtlasXCORDHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{AtlasXCORDVersion}/g, VERSION)
        .replace(/{AtlasXCORDHash}/g, gitHashShort)
        .replace(/{AtlasXCORDPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{AtlasXCORDIcon}":
                return <span key={`icon-AtlasXCORD-${i}`} className="vc-discord-dev-banner-icons"><AtlasXCORDIcon /></span>;
            case "{electronIcon}":
