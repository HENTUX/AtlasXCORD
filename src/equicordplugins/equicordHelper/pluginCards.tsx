...
import { isAtlasXCORDGuild, isAtlasXCORDSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of AtlasXCORD. Try updating!";
    ...
    if (required) {
        const tooltipText = p.required || !dependents.length
            ? "This plugin is required for AtlasXCORD to function."
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://AtlasXCORD.org/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isAtlasXCORD = isAtlasXCORDGuild(message.channel_id) && isAtlasXCORDSupport(message.author.id);
        if (!isAtlasXCORD) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://AtlasXCORD.org/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
