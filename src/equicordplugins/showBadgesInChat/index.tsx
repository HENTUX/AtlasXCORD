...
import { Devs, AtlasXCORDDevs } from "@utils/constants";
import { isAtlasXCORDPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "AtlasXCORDDonor":
            return (
                <span style={{ order: settings.store.AtlasXCORDDonorPosition }}>
                    {badges.getAtlasXCORDDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "AtlasXCORDContributer":
            return isAtlasXCORDPluginDev(author.id) ? (
                <span style={{ order: settings.store.AtlasXCORDContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="AtlasXCORD Contributor"
                        size={20}
                        src={"https://AtlasXCORD.org/assets/favicon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showAtlasXCORDDonor && <CheckBadge badge={"AtlasXCORDDonor"} author={author} />}
            {settings.store.showAtlasXCORDContributor && <CheckBadge badge={"AtlasXCORDContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, AtlasXCORDDevs.KrystalSkull],
