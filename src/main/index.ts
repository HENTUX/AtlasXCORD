...
if (!IS_VANILLA && !IS_EXTENSION) {
    app.whenReady().then(() => {
        ...
        protocol.handle("AtlasXCORD", ({ url: unsafeUrl }) => {
            let url = decodeURI(unsafeUrl).slice("AtlasXCORD://".length).replace(/\?v=\d+$/, "");
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[AtlasXCORD] Installed React Developer Tools"))
                    .catch(err => console.error("[AtlasXCORD] Failed to install React Developer Tools", err));
        } catch { }
