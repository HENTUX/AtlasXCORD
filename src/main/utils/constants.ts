...
export const DATA_DIR = process.env.AtlasXCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "AtlasXCORDData", suffix)
        : join(app.getPath("userData"), "..", "AtlasXCORD", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[AtlasXCORD] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
