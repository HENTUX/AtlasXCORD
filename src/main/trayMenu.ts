...
function createAtlasXCORDMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "AtlasXCORD",
            submenu: [
                {
                    label: "About AtlasXCORD",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update AtlasXCORD" : "Check for Updates",
                    click: () => sendToRenderer(IpcEvents.TRAY_CHECK_UPDATES)
                },
                {
                    label: "Repair AtlasXCORD",
...
export function patchTrayMenu(): void {
    ...
    Menu.buildFromTemplate = function (template: MenuItemConstructorOptions[]) {
        const alreadyPatched = template.some(item => item.label === "AtlasXCORD");
        if (isTrayMenu(template) && !alreadyPatched) {
            const insertIndex = findInsertIndex(template);
            const AtlasXCORDItems = createAtlasXCORDMenuItems();
            template.splice(insertIndex, 0, ...AtlasXCORDItems);
        }
