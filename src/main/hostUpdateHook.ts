...
/**
 * re-applies the AtlasXCORD patch to a freshly installed discord host
...
interface DiscordUpdaterModule {
    ...
    __AtlasXCORDTryInitWrapped?: boolean;
}
...
interface DiscordDesktopCore {
    ...
    __AtlasXCORDStartupWrapped?: boolean;
}

const error = (...args: unknown[]) => console.error("[AtlasXCORD:HostUpdate]", ...args);
...
const retainAtlasXCORD = (updater: DiscordHostUpdater, reason: string) => {
    try {
...
const attachToUpdater = (updater: DiscordHostUpdater | null | undefined) => {
    ...
    updater.on?.("host-updated", () => retainAtlasXCORD(updater, "host-updated"));
    ...
    if (typeof sync === "function") {
        ...
        updater.startCurrentVersionSync = (options?: StartCurrentVersionOptions) => {
            bound(options);
            try { retainAtlasXCORD(updater, "startCurrentVersionSync"); } catch (e) { error(e); }
        };
    }
    ...
    if (typeof async_ === "function") {
        ...
        updater.startCurrentVersion = async (queryOptions?: object, options?: StartCurrentVersionOptions) => {
            ...
            try { retainAtlasXCORD(updater, "startCurrentVersion"); } catch (e) { error(e); }
        };
    }
};

const wrapStartup = (coreExports: DiscordDesktopCore | null | undefined) => {
    if (!coreExports?.startup || coreExports.__AtlasXCORDStartupWrapped) return;
    coreExports.__AtlasXCORDStartupWrapped = true;
    ...
    coreExports.startup = function (opts, ...rest) {
        try {
            ...
            } else if (typeof updaterModule?.tryInitUpdater === "function" && !updaterModule.__AtlasXCORDTryInitWrapped) {
                /*
                 * updater not yet constructed at startup time. wrap the
                 * factory so we attach once vanilla creates it.
                 */
                updaterModule.__AtlasXCORDTryInitWrapped = true;
