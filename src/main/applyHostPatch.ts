    ...
    try {
        ...
        for (let i = undo.length - 1; i >= 0; i--) {
            try {
                ...
                console.error("[AtlasXCORD] Rollback step failed", cleanupErr);
            }
        }
        throw err;
    }
    ...
    try {
        for (const name of readdirSync(discordPath)) {
            ...
            try {
                ...
                console.error("[AtlasXCORD] Skipping unreadable sibling", name, statErr);
                continue;
            }
            if (!isDir) continue;
        ...
        console.error("[AtlasXCORD] Failed to scan for sibling versions", err);
        return null;
    }
