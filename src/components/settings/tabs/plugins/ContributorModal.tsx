...
import { AtlasXCORDDevsById, VencordDevsById } from "@utils/constants";
...
function ContributorModal({ user, modalProps }: { user: User; modalProps: RenderModalProps; }) {
    ...
    const plugins = useMemo(() => {
        ...
        const pluginsByAuthor = (VencordDevsById[user.id] || AtlasXCORDDevsById[user.id])
            ? allPlugins.filter(p => p.authors.includes(VencordDevsById[user.id] || AtlasXCORDDevsById[user.id]))
            : allPlugins.filter(p =>
    ...
    const ContributedHyperLink = <Link href="https://github.com/HENTUX/AtlasXCORD">contributed</Link>;
