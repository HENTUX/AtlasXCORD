...
import { AtlasXCORD_HELPERS, AtlasXCORDDevsById, GUILD_ID, KNOWN_ISSUES_CHANNEL_ID, SUPPORT_CHANNEL_ID, VencordDevsById } from "./constants";
...
export const isAtlasXCORDPluginDev = (id: string) => Object.hasOwn(AtlasXCORDDevsById, id);
export const shouldShowAtlasXCORDContributorBadge = (id: string) => isAtlasXCORDPluginDev(id) && AtlasXCORDDevsById[id].badge !== false;

export const isAnyPluginDev = (id: string) => Object.hasOwn(VencordDevsById, id) || Object.hasOwn(AtlasXCORDDevsById, id);
...
export function isAtlasXCORDGuild(id: string | null | undefined, isGuildId: boolean = false): boolean {
    if (!id) return false;
...
export function isAtlasXCORDSupport(userId: string | null | undefined): boolean {
    if (!userId) return false;

    const member = GuildMemberStore.getMember(GUILD_ID, userId);
    if (!member) return false;
    return member.roles.includes(AtlasXCORD_HELPERS) || false;
}
